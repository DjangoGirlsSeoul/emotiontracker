...
int pinNumbers[NUM_INPUTS] = {
  12, 8, 13, 15,    // top of makey makey board
...
const int outputC = 7;  // c  - space
const int outputD = 6; // d  - click
const int outputK = 14; // start s - 8 (D14)
const int outputM = 16; //  stop w - 6  (D16)
const int outputE = 23; // e - MOUSE_MOVE_UP
const int outputF = 22; // f - MOUSE_MOVE_DOWN
const int outputG = 21; // g - MOUSE_MOVE_LEFT
const int outputA = 20; // a - MOUSE_MOVE_RIGHT
const int outputB = 19; // b - MOUSE_LEFT
const int outputN = 18; // n(c1) - MOUSE_RIGHT
byte ledCycleCounter = 0;

// maping inputs to output for LED display
int pinNumbersOut[NUM_OUTPUTS] = {
  outputC, outputE, outputB, outputN,
  outputM, outputA, outputK, outputD, outputF, outputG
...
for (int i=0; i<NUM_INPUTS; i++) {
    if(no == i) {
      digitalWrite(NUM_OUTPUTS[no], HIGH);
      TXLED1;
      RXLED0;
    } else {
      digitalWrite(NUM_OUTPUTS[i], LOW);
      TXLED0;
      RXLED1;
    }
}
